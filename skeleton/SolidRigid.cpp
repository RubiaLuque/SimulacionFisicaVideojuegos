...
SolidRigid::SolidRigid(Vector3 pos, Vector3 lVel, Vector3 aVel, Vector4 color, double radius, double mass, Data::RIGIDS r)
{
	...
	if (r == Data::SUELO) {
		//PxRigidStatic* suelo = getPhysics()->createRigidStatic(PxTransform(pos));
		//PxShape* shape = CreateShape(PxBoxGeometry(100, 0.1, 100));
		//suelo->attachShape(*shape); //Se enlaza la caja con un solido rigido
		//getScene()->addActor(*suelo); //Se añade el solido rigido a la escena
		////Pintar el suelo
		...
		//PxRigidStatic* staticR = getPhysics()->createRigidStatic(PxTransform(pos));
		...
		//getScene()->addActor(*staticR); //Se añade el solido rigido a la escena
		////Pintar el suelo
		...
		//new_solid = getPhysics()->createRigidDynamic(PxTransform(pos));
